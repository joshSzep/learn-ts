...
import { createGreeting, parseGreetingArgs } from "../src/greeting.js";
...
describe("parseGreetingArgs", () => {
  ...
  it("supports the short excited flag", () => {
    expect(parseGreetingArgs(["Josh", "-e"])).toEqual({
      recipient: "Josh",
      excited: true,
    });
  });
  ...
  it("rejects unknown options", () => {
    expect(() => parseGreetingArgs(["--mystery"])).toThrow(CommanderError);
  });
});
